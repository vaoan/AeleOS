 ...
 *  * **`frame-ancestors 'none'`** is clickjacking protection: NOTHING may put
 *    this app in a frame, our own pages included. It replaces
 *    `X-Frame-Options` and is stricter than that header's `SAMEORIGIN`.
 *
 *    **It was `'self'` for a day, and it is closed again.** The widening on
 *    2026-08-26 had exactly one beneficiary: the complete-page preview framed
 *    `/{locale}/me/preview` to give a draft its own viewport. That preview is
 *    gone — the editor themes its own document with the page being built and
 *    hides its controls to show it, so there is no frame left to permit.
 *
 *    The widening is worth remembering for the way it was found rather than
 *    for what it allowed: `'none'` refused the frame outright, the frame
 *    rendered blank, and the violation went to a console where nothing
 *    asserts. It was caught by driving a browser, which is the only instrument
