...
export function SignOutControl({ label, redirectUrl }: SignOutControlProps) {
  ...
  return (
    <button
      ...
      {...tid("sign-out")}
      className="rounded-lg surface border-(--edge) px-4 py-2 text-sm font-medium transition-colors hover:bg-(--edge)/15 focus-visible:outline-2 focus-visible:outline-(--accent)"
    >
      {label}
    </button>
  );
