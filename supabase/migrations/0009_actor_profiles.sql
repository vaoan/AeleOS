...
create table public.actor_profiles (
  ...
  --                portrait?, radius?, heading_pad?, heading_image?,
  --                heading_fit?, heading_gap?, corners?, heading_corners? } —
  --                how the BLOCK chooses to look, form only and
...
begin
  ...
  if p_block ? 'style' then
    ...
    for v_key, v_value in select * from jsonb_each_text(p_block -> 'style') loop
      ...
      elsif v_key = 'portrait' then
        -- How large the avatar leaf draws its own portrait, independent of
        -- image_fit's crop. Absent and `m` are the same size — size-24, what
        -- every page carried before this key.
        if v_value not in ('s', 'm', 'l') then
          raise exception 'block %: unknown portrait size', p_path using errcode = '22023';
        end if;
