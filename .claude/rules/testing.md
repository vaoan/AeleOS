...
- **A dnd-kit modifier is a pure function of its args and is unit-tested as one (2026-09-17).** `tests/preview-follows-pointer.test.ts` builds the full modifier argument bag with a spread over nulls, feeds a real `MouseEvent` with client coordinates for the pointer case and a `KeyboardEvent` for the no-pointer case, and asserts the untouched-transform cases by identity (`toBe`), not equality — a modifier that returned a copy would re-render the overlay on every frame for nothing. Sabotaged to "return the transform" for the red run: 1 failed, 3 passed.
- **When a change REMOVES a prop, the new cases cannot go red against the old code by omitting it, and a green run there proves nothing (2026-09-16).** `drop-mark.test.tsx`'s place-mark cases passed unchanged against the component that still read `height`: an absent prop reached it as `undefined`, and jsdom drops an inline height whose value is not a length, so the old code rendered no `style` either. The red run came from mutating the SOURCE to the wrong behaviour (a `place` branch that sizes itself: 4 failed, 74 passed, restored by copy, 78 green), and the guard against re-threading the prop is the type — `DropMarkProps` has no `height` — which `pnpm typecheck` enforces and vitest never will (rule 40).
