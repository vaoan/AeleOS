...
- **A spec's status line is owed by the branch that delivers it, and the root `HISTORY.md` header with it (2026-09-16).** The carrd-style and palette-drag-to-add specs, the dragging spec's `DragOverlay` bullet, the blocks-and-grids spec's depth-three question and one root `HISTORY.md` header all still read "not built" or "unproven" ten days after shipping. Every delivering PR updated the root map and the feature `HISTORY.md`, which the gate demanded, and none touched the spec, which it did not. A status line closes like any other open question: the original text stays and a dated bold marker says what shipped and what is still owed.
- **`check:docs` and `check:agent-notes` compare `origin/main...HEAD` by default (2026-09-16).** On a branch with nothing committed both report a clean pass over zero files. The pre-commit hook (`.husky/pre-commit`) runs both with `--staged`, so a commit is gated; a manual run before committing is only honest with `--staged` after `git add`, and the root map's Commands list says so.
- **A closing marker on a spec's open question names what the closing case found INSTEAD, when that is a different fault (2026-09-17).** The dragging spec's long-page bullet closed with "the rectangles were never stale" and a pointer to the frozen-edge fix recorded in the drop-target-legibility spec's §7 addendum; a marker that only said "closed" would have hidden that the proof failed for a reason the question never asked, and the next reader would have re-derived it.
- **A picture-proof read-back that finds a fault is fixed on the same branch and re-photographed, and the account says the proof found it (2026-09-17).** The midline pictures for PR #93 had no preview pill in the frame; the read-back named it, the fix (`previewFollowsPointer`) landed with its own two guards, pictures 1, 2, 4 and 5 were re-captured and re-posted, and the feature `HISTORY.md` records the fault under the proof that surfaced it rather than as a separate entry — because what the next reader needs is that a read-back of the frame is what caught it.
