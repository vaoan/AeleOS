...
- **`check:docs` and `check:agent-notes` compare `origin/main...HEAD` by default (2026-09-16).** On a branch with nothing committed both report a clean pass over zero files. Run the scripts with `--staged` after `git add` to gate uncommitted work; the root map's Commands list says so.
