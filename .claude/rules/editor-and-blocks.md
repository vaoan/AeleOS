...
- **A Tailwind class that compiles to NOTHING is indistinguishable, in every test this repository has, from one that works.** → `docs/lessons/rules/36-a-tailwind-class-that-compiles-to-nothing-is-indistinguishab.md`
- **A write path's looseness is usually justified by a CONTROL, and the justification is void the moment a paste box exists.** → `docs/lessons/rules/37-a-write-path-s-looseness-is-usually-justified-by-a-control-a.md`
